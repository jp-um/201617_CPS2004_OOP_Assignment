 ...
 * 
 * This class represents the current game state.  Moreover
 * it does so by keeping a one dimensional array as follows
 ...
 *   6 | 7 | 8 
 * </pre>
 * Each position is initially empty and may be filled with an
 * X or O. 
 * 
 * We now keep the number of moves played - as a cache
 * for easier calculation of isFull, isEmpty methods.
 * 
...
public final class GameBoard {
	...
	/**
	 * The board size (max number of X and Os which
	 * we can fit on the board)
	 */
	private final int BOARD_SIZE = 9;
	
	/**
	 * The array holding the actual board state.
	 * We will populate this as the game goes by.
	 */
	private final Mark[] board = new Mark[BOARD_SIZE];  
	
	/**
	 * The number of moves played on this board.
	 * Never negative, =< BOARD_SIZE
	 */
	private int movesPlayed = 0;

	/**
	 * A list which contains all empty positions on the board.
	 * Updated upon playing at a particular position.
	 * We keep an attribute so we do not have to iterate
	 * (linear time) over the board each time to check for empty
	...
	/**
	 * Constructor which initialises
	 * the empty positions list to all
	...
	/**
	 * Gets the board state (the array containing
	 * Xs and 0s).  Note that this method actually
	 * returns a copy of the actual array so none
	 * of the players have a live reference to the
	 * actual board array (which they can change, hack 
	 * and cheat with).  Hence this method is a bit
	 * heavy and should be called sparingly.
	 * 
	 * @return The single dimensional array representing
	 * the board state.  Never null.
	 */
	...
	/**
	 * Checks if the board is empty, simply by checking
	 * the number of moves played on this board.
	 * @return true if the board is full, false otherwise
	 */
	...
	/**
	 * Gets the board size (this may actually
	 * be determined by calling length on the
	 * board array, but for convenience we offer
	 * this method).
	 * 
	 * @return An integer representing the board size
	 * (the one dimensional array).  Positive, and
	 ...
	 * 
	 * Note that this method will actually return 
	 * false if the position is out of range.  That
	 * is < 0 and >= BOARD_SIZE (currently at 9).
	 * 
	 * @param position The position to check for vacancy
	 * @return true if occupied, false otherwise (even out of range) 
	 ...
	 * 
	 * Particularly it checks if the tentative position is in 
	 * a valid range 0..(BOARD_SIZE-1).
	 * 
	 * Also it checks if there already is a sign in the tentative position.
	 *
	 * This method should be called by all players prior to
	 * returning their playing position.  The engine will disqualify
	 * any robot which does not comply to this validation.
	 * 
	 * @param tentativePosition the tentative position where to play
	 * @throws PositionOutOfRangeException If the tentative exception is out of range < 0 and >= BOARD_SIZE (9)
	 * @throws PositionAlreadyOccupiedException If the tentative position is already occupied.
	 */
	public void validatePlayTentative(final int tentativePosition) throws PositionOutOfRangeException, PositionAlreadyOccupiedException {
		// check range
		...
		if (isOccupied(tentativePosition)) {
			throw new PositionAlreadyOccupiedException("Position " + tentativePosition + " is already occupied by " + board[tentativePosition]);
		}

	}

	/**
	 * Displays the game board, with some funky characters.
	...
	/**
	 * Proper toString() method which is mostly used
	...
	/**
	 * Plays the position on the board. Before the actual 
	 * playing, the position is validated by calling the
	 * validate method above. 
	 * 
	 * Note that this method is default/package protected - so
	 * that no one outside the engine can call it.
	 * 
	 * A hacker might call this method in his play() and circumvent
	 * the engine.
	 * 
	 * @param positionToPlay The position to play
	 * @param mark The sign (either X or O) to place at the position
	 * @throws PositionOutOfRangeException If the position to play is out of range
	 * @throws PositionAlreadyOccupiedException If the position is already occupied 
	 */
	void playAtPosition(final int positionToPlay, final Mark mark) throws PositionOutOfRangeException, PositionAlreadyOccupiedException {
		// check
	 ...
	 * 
	 * When board is full this should return a list of size 0.
	 * Notice that we give a copy of the original emptyPositions
	 * so no one will have a live reference to it.
	 * 
	 * This method is much more performant than iterating
	 * through the collection and adding to a new arraylist
	 * 
